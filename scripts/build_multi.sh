...
# Check the first parameter for the image name
if [[ -z "$1" ]]; then
  echo "Please provide the image name as the first parameter"
  exit 1
fi
...
docker buildx build --platform linux/amd64,linux/arm64 -t lkup77/$1:$TAG_VERSION --push .
